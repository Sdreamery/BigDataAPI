...
//        SparkUtils.mockData(sc, sqlContext);
        ...
        System.out.println("task.getTaskParams()----" + task.getTaskParams());//时间范围(startDate,endDate)
    ...
    private static void generateTempAreaRoadFlowTable(SQLContext sqlContext) {
        ...
        String sql =
                "SELECT "
                        ...
                        + "road_id,"
                        + "count(*) car_count,"
                        //group_concat_distinct 统计每一条道路中每一个卡口下的车流量
                        + "group_concat_distinct(monitor_id) monitor_infos "
                        + "FROM tmp_car_flow_basic "
                        + "GROUP BY area_name,road_id"; //用区域+道路Id作为分组双键，防止每个区域的道路编号都是从1开始
...
//		df.show();
     ...
     */
    private static void  generateTempRoadFlowBasicTable(
            SQLContext sqlContext,
            JavaPairRDD<String, Row> areaId2DetailInfos,
            JavaPairRDD<String, Row> areaId2AreaInfoRDD) {
     ...
     * @return (areaId, row)
     */
    private static JavaPairRDD<String, Row> getInfosByDateRDD(
