package com.seanxia.spark.areaRoadFlow;
...
import com.seanxia.spark.util.NumberUtils;
...
public class MonitorOneStepConvertRateAnalyze {
    public static void main(String[] args) {
        ...
        long taskid = ParamUtils.getTaskIdFromArgs(args,
                Constants.SPARK_LOCAL_TASKID_MONITOR_ONE_STEP_CONVERT);
        ...
        for (Entry<String, Double> entry : entrySet) {
            System.out.println("行车轨迹： " +entry.getKey()+" =====> "+"道路转化率： "+entry.getValue());
        }
    }

    private static Map<String, Long> getRoadFlowCount(JavaPairRDD<String, Long> roadSplitRDD) {

        JavaPairRDD<String, Long> sumByKey = roadSplitRDD.reduceByKey(
     ...
     */
    private static Map<String, Double> computeRoadSplitConvertRate(
            Broadcast<String> roadFlowBroadcast, Map<String, Long> roadFlow2Count) {
        //0001，0002，0003，0004，0005
        String roadFlow = roadFlowBroadcast.value();
        String[] splitList = roadFlow.split(",");
//		List<String> roadFlowList = Arrays.asList(split);
        ...
        for (int i = 0; i < splitList.length; i++) {
            ...
            if (count != null) {
                ...
                if (i != 0 && lastMonitorCarCount != 0L) {
                    double rate = NumberUtils.formatDouble((double) count
                            / (double) lastMonitorCarCount, 2);
     ...
     */
    private static JavaPairRDD<String, Long> generateAndMatchRowSplit(
            JSONObject taskParam, final Broadcast<String> roadFlowBroadcast,
            JavaPairRDD<String, Row> car2RowRDD) {
        return car2RowRDD
                .groupByKey()
                .flatMapToPair(new PairFlatMapFunction<Tuple2<String, Iterable<Row>>, String, Long>() {
            private static final long serialVersionUID = 1L;
            ...
            public Iterable<Tuple2<String, Long>> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
                ...
                /**
                 * roadFlowBuilder保存到是本次车辆的轨迹是一组逗号分开的卡口id，组合起来就是这辆车的运行轨迹
                 */
                StringBuilder roadFlowBuilder = new StringBuilder();

                /**
                 * roadFlowBuilder怎么拼起来的？  rows是有顺序了，直接遍历然后追加到roadFlowBuilder就可以了吧。
                ...
                for (int i = 1; i <= split.length; i++) {
                    ...
                    //"helloshseanxiahahaxixihelloshseanxia" , "hello"
