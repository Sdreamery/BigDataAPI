package com.seanxia.spark.skynet;
...
import com.seanxia.spark.domain.TopNMonitor2CarCount;
...
import com.seanxia.spark.conf.ConfigurationManager;
import com.seanxia.spark.constant.Constants;
import com.seanxia.spark.dao.IAreaDao;
import com.seanxia.spark.dao.IMonitorDAO;
import com.seanxia.spark.dao.factory.DAOFactory;
import com.seanxia.spark.domain.Area;
import com.seanxia.spark.domain.MonitorState;
import com.seanxia.spark.domain.Task;
import com.seanxia.spark.domain.TopNMonitorDetailInfo;
import com.seanxia.spark.util.ParamUtils;
import com.seanxia.spark.util.SparkUtils;
 ...
 * --class com.seanxia.spark.skynet.MonitorFlowAnalyze
...
//			.set("spark.serializer", "org.apache.spark.serializer.KryoSerializer").registerKryoClasses(new Class[]{SpeedSortKey.class})
        ...
        /**
         * 按照卡口号分组，对应的数据是：每个卡扣号(monitor)对应的Row信息
		 ...
		 * "01","02" 指的是两个区域
		 */
        areaCarPeng(sqlContext, taskParamsJsonObject, "01", "02");

	 	/**
	 	 * 卡口碰撞分析，直接打印结果
	 	 *
	 	 */
        monitorCarPeng(sqlContext, taskParamsJsonObject);
     ...
     */
    private static void monitorCarPeng(SQLContext sqlContext, JSONObject taskParamsJsonObject) {
        ...
        JavaPairRDD<String, String> distinct1 = areaRDD1.mapToPair(
        ...
        JavaPairRDD<String, String> distinct2 = areaRDD2.mapToPair(
                new PairFunction<Row, String, String>() {
            private static final long serialVersionUID = 1L;
        ...
        distinct1.join(distinct2).foreach(new VoidFunction<Tuple2<String, Tuple2<String, String>>>() {
            ...
            public void call(Tuple2<String, Tuple2<String, String>> arg0)
                    throws Exception {
                System.out.println("carId ---- " + "\t" + arg0._1);
...
//		});
     ...
     *
     */
    private static void areaCarPeng(SQLContext sqlContext, JSONObject taskParamsJsonObject, String area1, String area2) {
        //得到01区域的数据放入rdd01
        JavaRDD<Row> cameraRDD01 =
                SparkUtils.getCameraRDDByDateRangeAndArea(sqlContext, taskParamsJsonObject, area1);

        //得到02区域的数据放入rdd02
        JavaRDD<Row> cameraRDD02 =
                SparkUtils.getCameraRDDByDateRangeAndArea(sqlContext, taskParamsJsonObject, area2);
...
//		});
    ...
    private static JavaPairRDD<String, String> addAreaNameByBroadCast2AggreageByMnonitor(
            JavaSparkContext sc, JavaPairRDD<String, String> aggregateMonitorId2DetailRDD) {
        //从数据中查询出来  区域信息
    ...
    private static JavaPairRDD<String, String> addAreaName2AggreageByMnonitor(SQLContext sqlContext, JavaPairRDD<String, String> aggregateMonitorId2DetailRDD) {
         ...
         */
        return areaId2AreaNameRDD.join(areaId2AggregateInfosRDD).mapToPair(
...
//                Object[] top10Cars = list.subList(0, 10).toArray();
    ...
    /**
     * 获取经常高速通过的TOPN卡扣 , 返回车辆经常高速通过的卡口List
     * <p>
...
//                    set.add(cameraId); 可以用set集合代替list判断去重
...
//                    }
                ...
                /**
                 * 优化：循环外面遍历list，避免了循环里每次的判断
                 */
                // list里的每个cameraId追加到tmpInfos中
                for (String s : list) {
                    tmpInfos.append("," + s);
                }
    ...
    /**
     * 将Row格式的RDD转换成K,V格式的RDD
     *
     * @param cameraRDD
     * @return
     */
    private static JavaPairRDD<String, Row> getMonitor2DetailRDD(JavaRDD<Row> cameraRDD) {
        JavaPairRDD<String, Row> monitorId2Detail = cameraRDD.mapToPair(
...
//		});
    ...
    private static JavaPairRDD<Integer, String> checkMonitorState(
        ...
        /**
         * 对每一个卡扣下面的信息进行统计，统计出来camera_count（这个卡扣下一共有多少个摄像头）,
         * camera_ids(这个卡扣下，所有的摄像头编号拼接成的字符串)
         * 返回：
...
//						System.out.println("standardCameraIdList:"+standardCameraIdList);
                                int abnormalCmeraCount = 0;//异常摄像头数
