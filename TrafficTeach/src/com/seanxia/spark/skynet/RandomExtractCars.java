...
//			.set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
     ...
     */
    private static JavaPairRDD<String, String> getCarTrack(
            long taskId, JavaPairRDD<String, Row> randomExtractCar2DetailRDD) {
        ...
        JavaPairRDD<String, String> carTrackRDD = groupByCar.mapToPair(
            ...
            public Tuple2<String, String> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
                ...
                return new Tuple2<String, String>(car, Constants.FIELD_DATE + "=" + date + "|"
                        + Constants.FIELD_CAR_TRACK + "=" + carTrack.substring(1));
            }
        });
    ...
    private static JavaPairRDD<String, Row> randomExtractCarInfo(
         ...
         * extractNums = 100 ， dateHourCountMap.size()为有多少不同的天数日期，就是多长
         */
        ...
        for (Entry<String, Map<String, Long>> entry : dateHourCountMap.entrySet()) {
            ...
            for (Entry<String, Long> hourCountEntry : hourCountMap.entrySet()) {
                ...
                /**
                 * 如果在这个时间段内抽取的车辆信息数量比这个时间段内的车流量还要多的话，
                 * 只需要将count的值赋值给hourExtractNum就可以
                 * 代码健壮性
                 */
...
//		}
        ...
        // 将每天每小时抽取索引的集合广播出去 dateHourExtractMap
        final Broadcast<Map<String, Map<String, List<Integer>>>> dateHourExtractBroadcast
                = sc.broadcast(dateHourExtractMap);
        /**
         * 在dateHour2DetailRDD中进行随机抽取车辆信息，
         * 首先第一步：按照date_hour进行分组，
         * 然后对组内的信息按照 dateHourExtractBroadcast参数抽取相应的车辆信息
        ...
        JavaPairRDD<String, String> extractCarRDD = dateHour2DetailRDD.groupByKey().flatMapToPair(
                new PairFlatMapFunction<Tuple2<String, Iterable<String>>, String, String>() {
                    ...
                    public Iterable<Tuple2<String, String>> call(Tuple2<String, Iterable<String>> t) throws Exception {
                        ...
                        // 使用广播变量 dateHourExtractBroadcast
        ...
        JavaPairRDD<String, Row> randomExtractCar2DetailRDD =
                extractCarRDD.join(car2DetailRDD).mapPartitionsToPair(
